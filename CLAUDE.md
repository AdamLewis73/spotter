...
**Phase 5 is feature-complete.** Every `V-##` it owns is met (V-10, V-11, V-12, V-26, V-28) and the D-22 checkpoint is discharged — `ScanLayout.boxFor` makes a saved word's position knowable, and the schema field carries into Phase 6. Save went live in Phase 6. What remains is the **tap-target work**: artboards 1b (ambiguity chips) and 1c (loupe), plus pinch-zoom on the frozen frame — `ux.md` calls all three necessary and none is built, so a tap must land on the glyph itself. Also a photographed fixture we own, since every committed one is generated.
