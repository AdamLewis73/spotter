...
**The overlay is built to artboard 1a**: the frame dims, the recognized text is **redrawn as type** into the rectangles `ScanLayout` measured (D-77), the tapped word takes a solid jade block, and a peek sheet gives the word, its glosses and two actions — with no reading, per D-47. `ScanProjection` handles the `ContentScale.Crop` transform and is tested in `:domain` apart from the interpolation, because on screen a wrong transform and a wrong interpolation look identical.
