...
**D-89 leaves a known defect in merged code:** `observeIsSaved` and `observeSaved` mean *the row exists*, and must come to mean *the row exists and has a live list membership*. Fix that before the Saved screen is built on the old meaning.
