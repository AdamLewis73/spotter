...
**Saved now means filed** (D-88, D-89): `observeIsSaved`, `observeSaved` and `find` all require a live `list_membership`, so a word taken out of its last list keeps its row and history and simply stops appearing. 18 instrumented cases cover it. **The loop is closed.** Saving opens a centred picker that stages its choices and writes nothing until *Add* (D-91), creating a list inline if there is none — which on a new install is the only route to saving anything. Lists already holding the word say so and are not a way to remove it; removal lives on the list screen and is **not built yet**. The kanji screen's Save works through the same picker (D-92). The picker has no artboard — the wireflow marks it NOT DESIGNED — so it follows the owner's spec rather than a drawing.
