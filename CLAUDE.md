...
**Phase 1 — Dictionary Builder: complete.** `tools/dictbuild/` builds a 99.7 MB `spotter.db` (30.3 MB gzipped) from four pinned sources in ~45 seconds, byte-reproducible from identical sources (D-58, D-64). `verify.py` passes 11 of 11 verification cases. **One case opened against it later: V-29** — a word's own readings are not separated by frequency, so 一人 leads with いちにん rather than ひとり. Fix is D-84; needs a rebuild and a `SCHEMA_VERSION` bump.
...
**Phase 6 — saved lists: in progress.** Its two entry checkpoints are settled. **D-79**: the app schedules reviews itself *and* exports to Anki — scheduler first, because building export first hands the retention loop away before there is one. **D-80**: soft delete (D-16) applies to rows the user deletes; `srs_state` and `scan_word` cascade with their parents; `updated_at` is universal. The standing user-data decisions (D-15–D-18, D-43) were reviewed and stand as written. **The schema is built and Save works**: `UserDatabase` v1 (`study_item`, `saved_list`, `list_membership`), schema JSON committed, 11 instrumented cases green, and the save/unsave/revive cycle driven on the emulator. **D-81** settles what Save stores — the top-ranked entry, the one whose glosses are on screen — because the peek shows no reading (D-47) while identity needs one (D-12). Next is the **Saved tab**, which needs the bottom-nav question asked first (D-36 vs D-73 vs D-61).
