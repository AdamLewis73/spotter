...
**Saved now means filed** (D-88, D-89): `observeIsSaved`, `observeSaved` and `find` all require a live `list_membership`, so a word taken out of its last list keeps its row and history and simply stops appearing. 18 instrumented cases cover it. **Interim consequence:** `save` alone creates the word without filing it, so until the list picker exists the save control does nothing visible — which is correct, not broken, and it is why every new screen shows its empty state. **The list picker is the next thing to build.**
