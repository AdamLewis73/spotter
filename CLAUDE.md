...
**Phase 4 — CameraX + ML Kit: complete.** The app **opens on a live viewfinder**, the shutter freezes a still (D-02), ML Kit reads the Japanese off it, and the text goes into the Phase 2 tokenize-and-look-up path — photograph a sign, tap a word, get its readings and senses. The camera is the launcher destination and the Phase 2 text box is a **debug path**, not a second front door (D-73); `/inspect` is unaffected. Its on-screen button is gone as of D-96 — the `query` intent extra is the way in. The ML Kit model is **bundled** at a measured ~14.8 MB per device (D-74).
...
**Phase 5 is feature-complete.** Every `V-##` it owns is met (V-10, V-11, V-12, V-26, V-28) and the D-22 checkpoint is discharged — `ScanLayout.boxFor` makes a saved word's position knowable, and the schema field carries into Phase 6. Save went live in Phase 6. A tap must land inside the glyph's own rectangle, ruby excepted; `ux.md`'s three **tap-target mitigations** — zoom, ambiguity chips, a tolerance radius — are unbuilt and now sit in `roadmap.md`'s deferred table with their cost. Still wanted: a photographed fixture we own, since every committed one is generated.

**Phase 6 — saved lists: complete.** Its two entry checkpoints are settled. **D-79**: the app schedules reviews itself *and* exports to Anki — scheduler first, because building export first hands the retention loop away before there is one. **D-80**: soft delete (D-16) applies to rows the user deletes; `srs_state` and `scan_word` cascade with their parents; `updated_at` is universal. The standing user-data decisions (D-15–D-18, D-43) were reviewed and stand as written. **The schema is built and Save works**: `UserDatabase` v1 (`study_item`, `saved_list`, `list_membership`), schema JSON committed, 13 instrumented cases green, and the save/unsave/revive cycle driven on the emulator. **D-81** settles what Save stores — the top-ranked entry, the one whose glosses are on screen — because the peek shows no reading (D-47) while identity needs one (D-12).

**A wireflow review then settled the rest of the phase's shape (D-85 to D-92)**, superseding three decisions. The app's own Scan · Saved · Review bar is drawn on **all three** destinations including the camera, and system back exits (D-90, superseding D-85). Typing a word moves off the camera to Saved (D-86). **Every saved word must be filed in at least one list** (D-88), through a picker that stages its choices and writes only on *Add* (D-91, superseding D-81's toggle). Unfiling keeps the word and its history but hides it until re-filed (D-89). **Kanji are study items in v1** (D-92, superseding D-01).

**The app shell is built.** Navigation Compose hosts three destinations and the app's own bar is drawn on all of them, camera included (D-90); back from Scan exits. **Saved** lists the user's lists with word counts and creates, renames, deletes and opens them. **Review** is a placeholder saying review is unbuilt — an empty queue and an unbuilt one look identical and mean opposite things. The design's due and learned counts are deliberately absent rather than shown as zero, since `srs_state` is Phase 7.

**Saved now means filed** (D-88, D-89): `observeIsSaved`, `observeSaved` and `find` all require a live `list_membership`, so a word taken out of its last list keeps its row and history and simply stops appearing. 18 instrumented cases cover it. **The loop is closed.** Saving opens a centred picker that stages its choices and writes nothing until *Add* (D-91), creating a list inline if there is none — which on a new install is the only route to saving anything. Lists already holding the word say so and are not a way to remove it; removal lives on the list screen — **hold a row, confirm, three seconds to undo** (D-93), and it never resets review progress, because FSRS already weights a returning word by the time elapsed. The kanji screen's Save works through the same picker (D-92). The picker has no artboard — the wireflow marks it NOT DESIGNED — so it follows the owner's spec rather than a drawing.

**Scan photos are built (D-94, D-95)** on `UserDatabase` **v2**, the first real migration (`AutoMigration`, `MigrationTestHelper`-tested). A photo is written only when *Add* files a word from it — once per shutter press, shared by every word filed from that frame, at captured size as WebP q80, **never resized**, because the stored word boxes are in that photo's pixels. List thumbnails decode the square around the word from the one file; no second image exists. Backup covers the database only, so a word without a photo is always a normal state. **Search is built (D-86, D-96)**: inside a list, *Add a word from search* opens search-as-you-type over written forms, and Save opens the picker with that list already ticked. The camera's debug search button is gone — the `query` intent extra is how `/inspect` gets in. **V-31** came out of it: a results row must show the reading its word screen leads with, tested on 孝 because 上手 stopped exhibiting the fault after D-84.
