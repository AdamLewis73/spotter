...
| **Word crops instead of full frames** | ~~Save just the tapped word's region rather than the whole photo~~ | **Closed, not deferred (D-94, D-95).** The whole photo is kept and the thumbnail *draws* the region around the word — the project owner does not want a second copy of the same image saved | Nothing to build beyond the thumbnail |
...
| **Kanji-only study items** | Let users add individual kanji to their SRS, not just words | ~~v1 studies words (D-01)~~ — **no longer deferred: D-92 puts them in v1.** D-49 sends a scanned lone kanji straight to the kanji screen, so deferring this meant a Save button that could not work | **Near zero, and now spent** — D-27's `type` discriminator was in the schema from day one for exactly this |
...
| **User-facing search** | A real "type a word" screen, as opposed to the debug text box the camera screen hides behind a flag | ~~Wanted, and asked for explicitly on 2026-08-24 — but what the *second* screen of a scanner-first app should be is a D-61 question~~ — **answered 2026-09-01 by D-86: it lives with Saved, not on the camera.** The screen already exists; what was deferred was only its home, and the wireflow settled it | **Near zero** — an entry point, not a build |
